setwd("F:/experiment/Conference Paper/ECML")
rm(list = ls())
...
savepath1      <- paste0("E:/experiment/ECML/ECML2021 Result/",paste0("EA2OKS-c-",Dataset[d_index],".txt"))
savepath2      <- paste0("E:/experiment/ECML/ECML2021 Result/",paste0("EA2OKS-c-all-",Dataset[d_index],".txt"))
  ...
  for (i in 1:length_tr)
  {
    ...
    for(r in 1:len_sigma)
    {
      ...
      if(fx*ylabel[order[i]] < para1_setting$varepsilon)
      {
        ...
        L[r]    <- L[r] + (para1_setting$varepsilon-fx*ylabel[order[i]])/(para1_setting$varepsilon+U)
